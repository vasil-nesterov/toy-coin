...
RSpec.describe Wallet do
  let(:private_key) { PrivateKey.load_from_file("#{ROOT_DIR}/spec/fixtures/alice_test.key") }
  ...
  let(:wallet) { Wallet.new(node: node, private_key: private_key) }

  describe "#to_h" do
    it "returns balance for a wallet with coins" do
      expect(wallet.to_h[:balance]).to eq(3_000)
    end
  end
